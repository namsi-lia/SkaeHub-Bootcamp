def generate_random_passwords(level):
  
    import string
    import random
    
    level_dict = {'low':6, 'middle':8, 'high':12}
...
def main():
    while True:
        ...
        if level.lower() == 'exit':
            ...
            print(generate_random_passwords('low'))
        elif check_level_input(level, 'middle'):
            print(generate_random_passwords('middle'))
        elif check_level_input(level, 'high'):
            print(generate_random_passwords('high'))
        else:
            print('Input error.')
